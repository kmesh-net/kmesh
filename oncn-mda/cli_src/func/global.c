...
static int init_mesh_map(struct mesh_map_info* const fds_map, const char* const pin_file_path,
						 const char* const map_name, struct bpf_create_map_attr* const map_attr)
{
	int ret = EOK;
...
static int init_mesh_prog(struct mesh_prog_info* const fds_prog, const char* const prog_name,
						  struct bpf_object_open_attr* const prog_attr,
						  enum bpf_attach_type attach_type, int attach_fd)
{
	int ret = EOK;
...
int init_fds(struct mesh_service_info* const fds, int cgroup_fd)
{
	...
	ret += init_mesh_map(&fds->map_fds[MESH_MAP_OPS_PROXY_MAP], pinmap_file_path[MESH_MAP_OPS_PROXY_MAP],
						to_str(SOCK_OPS_PROXY_MAP_NAME), &g_sock_ops_proxy_map_xattr);
	ret += init_mesh_map(&fds->map_fds[MESH_MAP_OPS_DUMP_I_MAP], pinmap_file_path[MESH_MAP_OPS_DUMP_I_MAP],
						to_str(SOCK_DUMP_MAP_I_NAME), &g_sock_dump_map);
	ret += init_mesh_map(&fds->map_fds[MESH_MAP_OPS_DUMP_DATA_MAP], pinmap_file_path[MESH_MAP_OPS_DUMP_DATA_MAP],
						to_str(SOCK_DUMP_CPU_ARRAY_NAME), &g_sock_dump_data_map);
	ret += init_mesh_prog(&fds->prog_fds[MESH_PROG_OPS], to_str(SOCK_OPS_NAME), &g_sock_ops_xattr,
						 BPF_CGROUP_SOCK_OPS, cgroup_fd);
	ret += init_mesh_prog_pin_file(&fds->prog_fds[MESH_PROG_OPS], pinprog_file_path[MESH_PROG_OPS]);
	ret += init_mesh_prog(&fds->prog_fds[MESH_PROG_REDIRECT], to_str(SOCK_REDIRECT_NAME), &g_sock_redirect_xattr,
						 BPF_SK_MSG_VERDICT, fds->map_fds[MESH_MAP_OPS_MAP].fd);
