...
static int get_input_port(const char* const src, struct input_filter_rule* const input_filter_rules)
{
	...
	int ret = strcpy_s(input_filter_rules->input_port[(input_filter_rules->input_port_num)++],
					   MAX_PORT_RANGE_LENGTH, src);
	if (ret == ERANGE_AND_RESET) {
...
static int set_cidr_filter_rule(struct input_cidr* const p, __u32 ipv4, __u32 mask)
{
	if (p->current_cidr_num + 1 > MAX_PARAM_LENGTH) {
		macli_log(ERR, "can not set accept ip rule, because the rule is too much! max rule num is %d\n",
				  MAX_PARAM_LENGTH);
		return FAILED;
	}
...
static int set_port_filter_rule(struct input_port* const p, __u32 begin_port, __u32 end_port)
{
	if (p->current_port_num + 1 > MAX_PARAM_LENGTH) {
		macli_log(ERR, "can not set accept ports rule, because the rule is too much! max rule num is %d\n",
				  MAX_PARAM_LENGTH);
		return FAILED;
	}
...
static int init_port_param(struct sock_param* const filter_rules,
						   const struct input_filter_rule* const input_filter_rules, bool is_accept)
{
...
static int init_uid_param(struct sock_param* const filter_rules,
						  const struct input_filter_rule* const input_filter_rules, bool is_accept)
{
...
static int chain_get_opt(int argc, char* const *argv,
						 struct input_filter_rule* const input_filter_rules, bool* is_accept)
{
	int opt;
