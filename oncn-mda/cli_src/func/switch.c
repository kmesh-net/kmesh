...
static const struct option g_disable_options[] = {
	{"help",		no_argument, NULL, 'h'},
	{NULL}
};
...
static int create_program_and_pinned(struct mesh_prog_info* const prog_info,
					const struct mesh_service_info* const fds)
{
...
static int detach_program(const struct mesh_service_info* const fds)
{
	for (int i = (int)MESH_PROG_NUM - 1; i >= 0; --i) {
		...
		if (fds->prog_fds[i].fd != -1 && fds->prog_fds[i].attach_fd != -1) {
			if (bpf_prog_detach2(fds->prog_fds[i].fd,
								 fds->prog_fds[i].attach_fd,
								 fds->prog_fds[i].attach_type) && errno != ENOENT) {
...
{
	int key = 0;
	if (bpf_map_update_elem(fds->map_fds[MESH_MAP_OPS_PARAM_MAP].fd,
							&key, filter_rules, BPF_ANY)) {
