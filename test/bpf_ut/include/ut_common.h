...
#define test_log(fmt, args...)                                                                                         \
    ({                                                                                                                 \
        ...
        *(suite_result_cursor++) =                                                                                     \
            2 + sizeof(____fmt) + ___bpf_narg(args) + (___bpf_narg(args) * sizeof(unsigned long long));                \
        *(suite_result_cursor++) = MKR_LOG_FMT;                                                                        \
        *(suite_result_cursor++) = sizeof(____fmt);                                                                    \
        bpf_memcpy(suite_result_cursor, ____fmt, sizeof(____fmt));                                                     \
        suite_result_cursor += sizeof(____fmt);                                                                        \
                                                                                                                       \
...
#define SETUP(progtype, name)  SEC(progtype "/test/" name "/setup")
