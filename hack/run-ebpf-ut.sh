...
EBPF_TEST_COMMAND="make -C /kmesh/test/bpf_ut run"
...
function run_ebpf_ut_in_docker() {
    ...
    build_kmesh $container_id # Build kmesh first to install dependencies defined in Dockerfile
...
if [ -z "$1" -o "$1" == "-d"  -o  "$1" == "--docker" ]; then
    if [[ "$*" == *"-v"* ]] || [[ "$*" == *"--verbose"* ]]; then
        EBPF_TEST_COMMAND="$EBPF_TEST_COMMAND V=1"
    fi
