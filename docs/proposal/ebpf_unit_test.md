...
2. **Go Test Driver Layer**: Responsible for loading eBPF programs, executing tests, and collecting results
...
1. **eBPF Unit Test Structure**:
   - **SETUP Section**: Prepare the test environment and configure the initial state
...
1. **Test Discovery**: Go test framework scans for eBPF object files (.o files) in specified directories
2. **Test Loading**: Load eBPF Collection for each test file
3. **Test Preparation**: Register necessary tail calls and initialize the test environment
4. **Program Classification**: Categorize programs into `setup`, `check`, and `pktgen` based on section names
5. **Test Execution**:
   - If `pktgen` exists, run it first to generate test data
   - Then run the `setup` program to set up the test environment
   - Finally, run the `check` program to verify results
6. **Result Collection**: Read test results from `suite_result_map`
7. **Result Reporting**: Parse results and generate reports using the Go testing framework
