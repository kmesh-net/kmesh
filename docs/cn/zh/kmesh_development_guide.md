...
基于k8s的云基础设施能够帮助应用实现敏捷的部署管理，但在应用流量编排方面有所欠缺；serviceMesh的出现很好地弥补了k8s流量编排的缺陷，与k8s互补。
...
- 网格数据面耗时分布
  ...
  网格耗时分布可以看出：sidecar架构引入大量时延开销，流量编排只占网格开销的10%，大部分开销在数据拷贝、多次的建链通信、上下文切换等。
...
- kmesh.json主要涉及服务初始编排规则及编排控制面ip信息；实际部署时，控制面地址会通过`ns:serviceName`从K8S集群中动态获取，控制面规则会实时刷新更新。
- 与mesh控制面之间通过grpc订阅
...
- 功能设计：支持tcp_proxy 类型的filter（流量过滤器），设计如图中L4:tcp_prxoy分支处理流程
  ...
  - 数据面进行消息匹配时，在filter_manager流程匹配当前对应的filter类型，如果是tcp_proxy filter，无需走L7路由治理逻辑，直接走L4治理逻辑，在tcp_proxy处理流程中进行转发决策。

    1）集群未配置WeightedCluster，则解析cluster_name，并ebpf尾调用到cluster_manager流程去做后续的endpoints负载均衡逻辑；

    2）集群配置WeightedCluster，即带有权重的一组cluster配置，则根据权重比例获取对应的cluster_name，并ebpf尾调用到cluster_manager流程去做后续的endpoints负载均衡逻辑。
...
- 编排规则转换成bpf map数据
  ...
  - proto-c数据 --> bpf map
    ...
    **基本思路：**
    ...
    2、数组型：成员val位置替换成outer_map idx， outer_map对应记录存储实际存放数组成员值的inner_map fd，注意这里的数组成员值又分两类：a）普通类型、b）复杂类型结构。
    ...
    3. 某个具体filter_chains访问方式：
       ...
       访问第二条filter_chains时，根据idx5在map_in_map中找到真正存储filter_chains结构的内部mapFd，再查找返回内部map对应的第一条记录的val，就是filter_chains信息。
