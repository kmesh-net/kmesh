...
2. **Go 测试驱动层**：负责加载 eBPF 程序、执行测试和收集结果
3. **结果通信层**：使用 Protocol Buffer 定义的结构进行测试结果传递

##### 核心组件

1. **eBPF 单元测试结构**：
   - **SETUP 段**：准备测试环境，配置初始状态
...
1. **测试发现**：Go 测试框架扫描指定目录下的 eBPF 对象文件（.o 文件）
2. **测试加载**：为每个测试文件加载 eBPF Collection
3. **测试准备**：注册必要的 tail calls，初始化测试环境
4. **程序分类**：根据段名将程序分为 `setup`、`check` 和 `pktgen` 三类
5. **测试执行**：
   - 如果存在 `pktgen`，首先运行它生成测试数据
   - 然后运行 `setup` 程序设置测试环境
   - 最后运行 `check` 程序验证结果
6. **结果收集**：从 `suite_result_map` 中读取测试结果
7. **结果报告**：解析结果并使用 Go 测试框架生成报告
