...
func markPodCgroupClassID(client kubernetes.Interface, pod *v1.Pod) error {
...
func recordPodIp(targetmap *ebpf.Map, preResult *cniv1.Result, containerID string) error {
...
func cleanPodIp(targetmap *ebpf.Map, containerID string) {
	var totalNum uint32
	var keyContainer kmeshManagerKey
	var keyIP kmeshManagerKey
	var index uint32
...
func enableKmeshControl(client kubernetes.Interface, pod *v1.Pod, preResult *cniv1.Result, containerID string) error {
	if err := markPodCgroupClassID(client, pod); err != nil {
		return err
	}
	...
	if err = recordPodIp(recordmap, preResult, containerID); err != nil {
		cleanPodIp(recordmap, containerID)
	}
...
func CmdDelete(args *skel.CmdArgs) error {
	...
	cleanPodIp(recordmap, args.ContainerID)
	return nil
}
