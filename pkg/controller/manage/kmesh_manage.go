...
func (c *KmeshManageController) processItems() {
	...
	if queueItem.action == ActionAddAnnotation {
		err = c.handleAddAnnotation(queueItem.pod)
	} else if queueItem.action == ActionDeleteAnnotation {
		err = c.handleDelAnnotation(queueItem.pod)
	}
...
func (c *KmeshManageController) handleAddAnnotation(pod *corev1.Pod) error {
	return retry.OnError(retry.DefaultRetry, func(err error) bool {
		return true
	}, func() error {
		return addKmeshAnnotation(c.client, pod)
	})
}

func (c *KmeshManageController) handleDelAnnotation(pod *corev1.Pod) error {
	return retry.OnError(retry.DefaultRetry, func(err error) bool {
		return true
	}, func() error {
		return delKmeshAnnotation(c.client, pod)
	})
}
