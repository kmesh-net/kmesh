...
// Create a test netns, link an old XDP program on veth0 created inside the netns
func newTestNetNsWithVethPeerIndex(t *testing.T) (uint64, ns.NetNS) {
	...
	testNs.Do(func(_ ns.NetNS) error {
		...
		if peer, err := netlink.LinkByName("veth1"); err != nil {
			t.Fatal(err)
		} else if err = netlink.LinkSetUp(peer); err != nil {
			t.Fatal(err)
		} else if peer.Attrs().Index > veth.Attrs().Index {
			targetIndex = uint64(peer.Attrs().Index)
		} else {
			targetIndex = uint64(veth.Attrs().Index)
		}
	...
	return targetIndex, testNs
}

func Test_getVethPeerNum(t *testing.T) {
	patches := gomonkey.NewPatches()
	defer patches.Reset()
	targetIndex, testNetNs := newTestNetNsWithVethPeerIndex(t)
	patches.ApplyFunc(ns.GetNS, func(_ string) (ns.NetNS, error) {
		return testNetNs, nil
	})
...
func Test_TC(t *testing.T) {
	patches := gomonkey.NewPatches()
	defer patches.Reset()
	_, testNetNs := newTestNetNsWithVethPeerIndex(t)
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			if err := linkTc(tt.args.netNsPath, tt.args.tcProgFd); (err != nil) != tt.wantErr {
				t.Errorf("linkTc() error = %v, wantErr %v", err, tt.wantErr)
			}
			if err := unlinkTc(tt.args.netNsPath, tt.args.tcProgFd); (err != nil) != tt.wantErr {
