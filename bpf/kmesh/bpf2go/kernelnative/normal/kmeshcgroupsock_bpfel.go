...
type KmeshCgroupSockBpfSock struct {
	BoundDevIf     uint32
	Family         uint32
	Type           uint32
	Protocol       uint32
...
type KmeshCgroupSockBpfSockTuple struct {
	Ipv4 struct {
		Saddr uint32
		Daddr uint32
		Sport uint16
...
type KmeshCgroupSockOperationUsageData struct {
	StartTime     uint64
	EndTime       uint64
	PidTgid       uint64
...
type KmeshCgroupSockOperationUsageKey struct {
	SocketCookie  uint64
	OperationType uint32
	_             [4]byte
}
