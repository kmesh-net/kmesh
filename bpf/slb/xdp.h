...
static inline address_t* map_lookup_tuple_ct(const tuple_t* tuple)
{
	return kmesh_map_lookup_elem(&map_of_tuple_ct, tuple);
}

static inline int map_update_tuple_ct(const tuple_t* tuple, address_t* target)
{
	return kmesh_map_update_elem(&map_of_tuple_ct, tuple, target);
}

static inline int map_delete_tuple_ct(const tuple_t* tuple)
{
						...
						__u32 flags)
{
	...
	: [ret]"=r"(ret), [sum]"=r"(sum)
	: [ctx]"r"(ctx), [off]"r"(off),
	[offmax]"i"(CTX_OFF_MAX), [errno]"i"(-EINVAL)
	: "r1", "r2");
	if (!ret)
...
static inline int xdp_l3_dnat(void* data, void* data_end, __u32 daddr)
{
	struct iphdr* iph;
...
static inline int xdp_l4_dnat(void * data, void * data_end, __u32 off_set, __u32 dport)
{
	struct tcphdr* tcp;
	tcp = data + off_set;
	...
	tcp->dest = (__be16)dport;
	return XDP_PASS;
}


static inline int parse_xdp_address(struct xdp_md* xdp_ctx,
					  bool is_ipv6,
					  address_t* src_addr,
					  address_t* dst_addr)
{
...
static void trace_nat_info(const address_t *src_address, const address_t *origin_dst, const address_t *backend)
{
...
static inline int xdp_dnat(struct xdp_md* xdp_ctx,
			 ...
			 bool add_tc)
{
	...
	trace_nat_info(src_address, origin_dst, backend);
	...
	sum_diff = bpf_csum_diff(&origin_dst->ipv4, sizeof(origin_dst->ipv4), &backend->ipv4, sizeof(backend->ipv4), 0);
